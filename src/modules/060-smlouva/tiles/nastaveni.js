// src/modules/060-smlouva/tiles/nastaveni.js
// Nastavení číslování smluv
...
export default async function render(root) {
  ...
  const initialData = configs || {
    ...
    zero_padding: 4,
    separator: '-',
    format_template: '{PREFIX}-{YEAR}-{NUMBER}',
    active: true
  };
  ...
  headerDiv.innerHTML = `
    <h2 class="text-2xl font-bold mb-2">Nastavení číslování smluv</h2>
    <p class="text-gray-600">
      Zde můžete nastavit formát a číselnou řadu pro automatické generování čísel smluv.
      Číslo smlouvy může obsahovat prefix (např. číslo nemovitosti/jednotky), rok a pořadové číslo.
    </p>
  `;
  ...
  const fields = [
    {
      ...
      type: 'text',
      required: true,
      help: 'Prefix pro číslo smlouvy (např. "SML", "NEM01", "JEDN05"). Můžete použít číslo nemovitosti nebo jednotky.',
    ...
    },
    {
      ...
      type: 'text',
      maxLength: 10,
      help: 'Znak pro oddělení částí čísla (např. "-", "/", "_")',
      section: 'Formát číslování'
    },
    {
      key: 'format_template',
      ...
      required: true,
      options: [
        { value: '{PREFIX}-{YEAR}-{NUMBER}', label: 'PREFIX-ROK-ČÍSLO (např. SML-2025-0001)' },
        { value: '{PREFIX}-{NUMBER}', label: 'PREFIX-ČÍSLO (např. SML-0001)' },
        { value: '{PREFIX}{SEPARATOR}{YEAR}{SEPARATOR}{NUMBER}', label: 'PREFIX/ROK/ČÍSLO (s vlastním oddělovačem)' },
        { value: '{PREFIX}{NUMBER}', label: 'PREFIXČÍSLO (bez oddělovače, např. SML0001)' },
        { value: '{YEAR}{SEPARATOR}{PREFIX}{SEPARATOR}{NUMBER}', label: 'ROK-PREFIX-ČÍSLO (např. 2025-SML-0001)' }
      ],
      help: 'Vyberte formát čísla smlouvy',
      section: 'Formát číslování'
    },
    {
      key: 'start_number',
    ...
    },
    {
      ...
      label: 'Aktivní',
      type: 'checkbox',
      help: 'Pokud je aktivní, bude tato konfigurace použita pro generování čísel',
      section: 'Stav'
    }
  ];
  ...
  previewDiv.className = 'mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg';
  previewDiv.innerHTML = `
    <h3 class="font-semibold mb-2">Náhled čísla smlouvy:</h3>
    <div id="number-preview" class="text-xl font-mono font-bold text-blue-700">-</div>
  `;
  ...
  function updatePreview(formData) {
    ...
    let result = formData.format_template || '{PREFIX}-{YEAR}-{NUMBER}';
    result = result.replace('{PREFIX}', formData.prefix || 'SML');

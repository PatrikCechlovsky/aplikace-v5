...
export async function render(root) {
  try {
    setBreadcrumb(document.getElementById('crumb'), [
      ...
      { icon: 'building-2', label: 'Bytový dům' }
    ]);
  } catch (e) { /* ignore if crumb missing */ }

  root.innerHTML = `<div id="commonactions" class="mb-4"></div><div id="property-table"></div>`;

  // Načti pouze bytové domy
  const { data, error } = await listProperties({ type: 'bytovy_dum', showArchived, limit: 500 });
  if (error) {
  ...
  function drawActions() {
    ...
    renderCommonActions(ca, {
      ...
      userRole,
      handlers: {
        onAdd: () => navigateTo('#/m/040-nemovitost/f/chooser?returnType=bytovy_dum'),

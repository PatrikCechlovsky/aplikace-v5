...
export async function render(root) {
  try {
    setBreadcrumb(document.getElementById('crumb'), [
      ...
      { icon: 'briefcase', label: 'Administrativní budova' }
    ]);
  } catch (e) { /* ignore if crumb missing */ }

  root.innerHTML = `<div id="commonactions" class="mb-4"></div><div id="property-table"></div>`;

  // Načti pouze administrativní budovy
  const { data, error } = await listProperties({ type: 'admin_budova', showArchived, limit: 500 });
  if (error) {
  ...
  const columns = [
    ...
    { key: 'pocet_podlazi', label: 'Podlaží', width: '10%' },
  ...
  function drawActions() {
    ...
    renderCommonActions(ca, {
      ...
      userRole,
      handlers: {
        onAdd: () => navigateTo('#/m/040-nemovitost/f/chooser?returnType=admin_budova'),

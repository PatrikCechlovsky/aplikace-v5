...
export async function render(root, params) {
  ...
  root.innerHTML = `
    ...
    <div class="p-4 bg-white rounded-2xl shadow">
      <h2 class="text-xl font-semibold mb-4">Detail nemovitosti</h2>
      <p class="text-slate-600 mb-4">Zde bude zobrazen detail nemovitosti${id ? ` ID: ${id}` : ''}.</p>
  ...
  if (ca) {
    ...
    renderCommonActions(ca, {
      ...
      handlers: {
        onEdit: () => {
          navigateTo(`#/m/040-nemovitost/f/edit${id ? '?id=' + id : ''}`);
        }
      }
    });
  }

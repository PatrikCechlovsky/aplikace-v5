...
// Použití: renderForm(root, fields, data, onSubmit)
...
export function renderForm(root, fields, data = {}, onSubmit) {
  if (!root) return;
...
function genInput(f, val) {
  ...
  switch (f.type) {
    ...
    case "number":
    case "date":
      return `<input type="${f.type}" id="${id}" name="${f.key}" class="border rounded px-2 py-1 w-full" value="${escapeHtml(val)}" ${f.required ? "required" : ""} />`;
    case "textarea":
      return `<textarea id="${id}" name="${f.key}" class="border rounded px-2 py-1 w-full" rows="3" ${f.required ? "required" : ""}>${escapeHtml(val)}</textarea>`;
    case "select":
      return `<select id="${id}" name="${f.key}" class="border rounded px-2 py-1 w-full" ${f.required ? "required" : ""}>
        ${f.options.map(opt => `<option value="${opt.value}" ${val == opt.value ? "selected" : ""}>${opt.label}</option>`).join("")}
      </select>`;
    case "checkbox":
      return `<input type="checkbox" id="${id}" name="${f.key}" class="mr-2 align-middle" ${val ? "checked" : ""} />`;
    case "checkbox-group":
      return `<div class="flex flex-col gap-1">
        ${f.options.map(opt => `
          <label class="inline-flex items-center gap-2">
            <input type="checkbox" name="${f.key}" value="${opt.value}" ${Array.isArray(val) && val.includes(opt.value) ? "checked" : ""} />
            ${opt.label}
          </label>`).join("")}
      </div>`;
    default:
      return `<input type="text" id="${id}" name="${f.key}" class="border rounded px-2 py-1 w-full" value="${escapeHtml(val)}" />`;
  }
}
